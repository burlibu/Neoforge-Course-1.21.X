package net.burlibu.mccourse.worldgen;
...
public class ModPlacedFeatures {
    ...
    public static void bootstrap(BootstrapContext<PlacedFeature> context) {
        ...
        register(context, EBONY_PLACED_KEY, configuredFeatures.getOrThrow(ModConfiguredFeatures.EBONY_KEY),
                VegetationPlacements.treePlacement(PlacementUtils.countExtra(3, 0.1f, 2))); // spawnano 3 e ce un 10% che ne spawnino altri due
                        //ModBlocks.EBONY_SAPLING.get())); // TODO se lo tolgo gli alberi spawnano sopra di loro. perchè il sapling è un bush block e ha certe condizioni
        ...
        register(context, END_BLACK_OPAL_ORE_PLACED_KEY, configuredFeatures.getOrThrow(ModConfiguredFeatures.END_BLACK_OPAL_ORE_KEY),
                ModOrePlacements.commonOrePlacement(9,
                        HeightRangePlacement.uniform(VerticalAnchor.absolute(-64), VerticalAnchor.absolute(80))));
    // uniform significa che hanno la stessa distribuzione per ogni y nel range, mentre triangle ce ne sono di piu al centro
